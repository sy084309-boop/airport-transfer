      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div>
            ...
            <div><label className="block text-[10px] text-fog mb-0.5">年</label><div className="flex items-center bg-charcoal rounded-lg border border-white/5"><button onClick={()=>setYear(y=>y-1)} className="px-2 py-2 text-fog hover:text-ivory text-xs">&larr;</button><span className="flex-1 text-center text-sm font-medium text-ivory">{year}</span><button onClick={()=>setYear(y=>y+1)} className="px-2 py-2 text-fog hover:text-ivory text-xs">&rarr;</button></div></div>
            <div><label className="block text-[10px] text-fog mb-0.5">月</label><select value={month} onChange={e=>setMonth(e.target.value)} className="input-premium w-full text-sm text-center">{Array.from({length:12},(_,i)=>String(i+1).padStart(2,'0')).map(m=><option key={m}>{m}月</option>)}</select></div>
            <div><label className="block text-[10px] text-fog mb-0.5">日</label><select value={day} onChange={e=>setDay(e.target.value)} className="input-premium w-full text-sm text-center">{Array.from({length:31},(_,i)=>String(i+1).padStart(2,'0')).map(d=><option key={d}>{d}日</option>)}</select></div>
            <div><label className="block text-[10px] text-fog mb-0.5">時</label><select value={hour} onChange={e=>setHour(e.target.value)} className="input-premium w-full text-sm text-center">{HOURS.map(h=><option key={h}>{h}時</option>)}</select></div>
            <div><label className="block text-[10px] text-fog mb-0.5">分</label><select value={minute} onChange={e=>setMinute(e.target.value)} className="input-premium w-full text-sm text-center">{MINUTES.map(m=><option key={m}>{m}分</option>)}</select></div>
          </div></div>
          ...
          <div><label className="block text-xs text-mist mb-2 uppercase tracking-wider">加值服務</label>
            ...
            {showServices&&<div className="space-y-1.5 ml-4 mt-2">{[{l:'嬰兒座椅 0-1歲',p:200},{l:'兒童座椅 1-4歲',p:200},{l:'增高座墊 4-8歲',p:150},{l:'搬行李上樓(件)',p:100},{l:'中途等待(30分)',p:300},{l:'攜帶寵物',p:100}].map((s,i) => {const qty = addonQtys[i] || 0; return (
              <div key={s.l} className="flex items-center justify-between">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input type="checkbox" checked={qty > 0}
                    onChange={() => setAddonQtys(prev => qty > 0 ? { ...prev, [i]: 0 } : { ...prev, [i]: 1 })}
                    className="accent-gold" />
                  <span className="text-xs text-mist">{s.l}<span className="text-gold ml-1">+${s.p}</span></span>
                </label>
                <div className="flex items-center gap-1">
                  <button type="button"
                    onClick={() => setAddonQtys(prev => ({ ...prev, [i]: Math.max(0, (prev[i] || 0) - 1) }))}
                    disabled={qty <= 0}
                    className="w-6 h-6 rounded border border-white/10 text-fog hover:text-ivory hover:border-white/20 text-xs leading-none disabled:opacity-30">−</button>
                  <span className="text-xs text-ivory w-5 text-center tabular-nums">{qty}</span>
                  <button type="button"
                    onClick={() => setAddonQtys(prev => ({ ...prev, [i]: (prev[i] || 0) + 1 }))}
                    className="w-6 h-6 rounded border border-white/10 text-fog hover:text-ivory hover:border-white/20 text-xs leading-none">+</button>
                </div>
              </div>
            )})}</div>}
          </div>
