      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          <div><label className="block text-xs text-mist mb-2 uppercase tracking-wider">加值服務</label>
            ...
            {showServices&&<div className="grid grid-cols-3 gap-2 ml-4 mt-2">{[{l:'嬰兒座椅 0-1歲',p:200},{l:'兒童座椅 1-4歲',p:200},{l:'增高座墊 4-8歲',p:150},{l:'搬行李上樓(件)',p:100},{l:'中途等待(30分)',p:300},{l:'攜帶寵物',p:100}].map((s,i)=>{const qty=addonQtys[i]||0;return(<div key={s.l} className="flex flex-col items-center gap-1 p-2 rounded-lg border border-white/5"><label className="flex items-center gap-1 cursor-pointer"><input type="checkbox" checked={qty>0} onChange={()=>setAddonQtys(prev=>qty>0?{...prev,[i]:0}:{...prev,[i]:1})} className="accent-gold"/><span className="text-xs text-mist">{s.l}</span></label><span className="text-[10px] text-gold">+${s.p}</span><div className="flex items-center gap-1"><button type="button" onClick={()=>setAddonQtys(prev=>({...prev,[i]:Math.max(0,(prev[i]||0)-1)}))} disabled={qty<=0} className="w-5 h-5 rounded border border-white/10 text-fog hover:text-ivory hover:border-white/20 text-xs leading-none disabled:opacity-30">−</button><span className="text-xs text-ivory w-4 text-center">{qty}</span><button type="button" onClick={()=>setAddonQtys(prev=>({...prev,[i]:(prev[i]||0)+1}))} className="w-5 h-5 rounded border border-white/10 text-fog hover:text-ivory hover:border-white/20 text-xs leading-none">+</button></div></div>)})}</div>}
          </div>
